    ...
    >
      <CardContent sx={{ p: 3 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <IconButton size="small" onClick={stopAnd(() => onUpvote(idea.id))} sx={{ p: 0.5 }}>
              <KeyboardArrowUp sx={{ fontSize: 18 }} />
            </IconButton>
            <Typography sx={{ fontWeight: 700, fontSize: 13, minWidth: 24, textAlign: 'center' }}>
              {idea.upvotes}
            </Typography>
            <IconButton size="small" onClick={stopAnd(() => onDownvote(idea.id))} sx={{ p: 0.5 }}>
...
export default function CommunityPage() {
  ...
  return (
    <Box sx={{ maxWidth: 980, mx: 'auto', width: '100%' }}>
      ...
      {/* Post detail modal (social-feed style) */}
      <Dialog
        ...
        <DialogContent sx={{ display: 'grid', gap: 2, pt: 1 }}>
          {activeIdea && (
            <Box
              sx={{
                display: 'grid',
                gridTemplateColumns: { xs: '1fr', md: activeIdea.imageUrl ? '1.15fr 0.85fr' : '1fr' },
                gap: 2,
                minHeight: '64vh',
              }}
            >
              <Box sx={{ display: 'grid', gap: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  ...
                  <Box>
                    ...
                    <Typography sx={{ fontSize: 12, color: '#9ca3af' }}>
                      {timeAgo(activeIdea.createdAt)} • ELO {activeIdea.authorElo}
                    </Typography>
                  </Box>
                </Box>

                <Typography sx={{ fontWeight: 900, fontSize: 20, lineHeight: 1.25 }}>
                  {activeIdea.title}
                </Typography>
                {activeIdea.caption && (
                ...
                {!!activeIdea.tags?.length && (
                  <Box sx={{ display: 'flex', gap: 0.75, flexWrap: 'wrap' }}>
                    {activeIdea.topic && (
                      <Chip key={`topic-${activeIdea.topic}`} label={activeIdea.topic} size="small" sx={{ bgcolor: '#ecfeff', border: '1px solid #ccfbf1', color: '#0f766e', fontWeight: 700 }} />
                    )}
                    {activeIdea.tags.map((t) => (
                      <Chip key={t} label={t} size="small" sx={{ bgcolor: '#f8fafc', border: '1px solid #e5e7eb' }} />
                    ))}
                  </Box>
                )}
                ...
                {activeIdea.notes && (
                  <Card elevation={0} sx={{ border: '1px solid #e5e7eb', borderRadius: 2 }}>
                    <CardContent sx={{ p: 2 }}>
                      ...
                      <Typography sx={{ color: '#6b7280', whiteSpace: 'pre-wrap', lineHeight: 1.6 }}>
                        {activeIdea.notes}
                      </Typography>
                    </CardContent>
                  </Card>
              ...
              <Card elevation={0} sx={{ border: '1px solid #e5e7eb', borderRadius: 2, display: 'flex', flexDirection: 'column', minHeight: 380 }}>
                <CardContent sx={{ p: 2, flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <Typography sx={{ fontWeight: 900, mb: 1 }}>
                    Comments ({activeIdea.comments?.length || 0})
                  </Typography>
                    ...
                    {(activeIdea.comments || []).length === 0 ? (
                      ...
                      (activeIdea.comments || []).map((c) => (
                        <Box key={c.id} sx={{ display: 'flex', gap: 1 }}>
                          <Avatar src={c.authorAvatar} sx={{ width: 28, height: 28, bgcolor: '#111', fontSize: 12 }}>
                            {!c.authorAvatar ? initials(c.author) : null}
                          </Avatar>
                          <Box sx={{ flex: 1 }}>
                            <Typography sx={{ fontWeight: 700, fontSize: 13 }}>
                              {c.author} <Box component="span" sx={{ color: '#9ca3af', fontWeight: 500 }}>{timeAgo(c.createdAt)}</Box>
                            </Typography>
                            <Typography sx={{ color: '#374151', lineHeight: 1.5, fontSize: 14 }}>
                              {c.text}
                            </Typography>
                          </Box>
                        </Box>
